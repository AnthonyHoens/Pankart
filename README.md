# Projet Panꓘart

> Il s'agit d'un Projet Client à réaliser pour le cours de Design Web : [hepl-dw/Projet-Pankart](https://github.com/hepl-dw/projet-pankart)

## Présentation

![Pankart logo](pankart_petit.jpg)

**Qui sont-ils ?**
PanKart, jeune groupe belge issu de la province de Luxembourg, chante en français, mais sa musique est un langage universel. Ses sonorités sont celtiques, rock et festives. Ses textes, **engagés** et **dénonciateurs** des choses qui, quand elles ne tournent pas rond, tournent en rond…

> [Vidéo Brief Clip](https://www.youtube.com/watch?v=LiKDvz-PMRM)

## Public cible

- Tout public confondu
- Pas les pré-ados !

## Cahier des charges et Briefs

1. _Retrouvez PanKart sur :_
    1. sites
    - [Gigstarter PanKart](https://www.gigstarter.be/fr-BE/artistes/pankart)
    - [Youtube PanKart](https://www.youtube.com/channel/UC7XhVCykTsQEJp6P0eTVfmQ)
    - [Lampi PanKart](https://lampli.be/artist-profile/pankart/)
    2. Réseaux Sociaux
    - [FaceBook PanKart](https://www.facebook.com/PanKart/)
    - [instagram PanKart](https://www.instagram.com/pankartmusicband/)
    3. Streaming
    - [Spotify](https://open.spotify.com/album/15EDw1ExEbLvtPYgWngsFk)
    - [Deezer](https://www.deezer.com/fr/track/934885562?utm_source=deezer&utm_content=track-934885562&utm_term=108514421_1587654068&utm_medium=web)
    - [Apple Music](https://music.apple.com/be/album/pankart-ep/1509177982?l=fr)
    - [Amazon](https://www.amazon.fr/PanKart-Pankart/dp/B08797KN6B/ref=sr_1_1?dchild=1&keywords=Pankart&qid=1587653912&s=dmusic&search-type=ss&sr=1-1)
    - [Google Play](https://play.google.com/store/apps/theme/promotion_gpm_shutdown_ctp)
    - [SoundCloud](https://soundcloud.com/pankartband)


2. Informations sur le groupe

    - Premier EP sorti en avril 2020
    - Concours européen remporté en décembre 2020


3. Charte graphique

    - P et K majuscule et K à l'envers **Panꓘart**
    - 2 couleurs (celles du LOGO) 
    - Choix des couleurs personnalisés pour le site


4. Souhaits de choses qu'ils souhaitent retrouver

    - nom de domaine : pankart.be
    - Site web responsive
    - Peut être mis à jour sans l'aide d'un tech (CMS : Wordpress)
    - construire en [éco conception](https://eco-mobile.org/comment-et-pourquoi-creer-un-site-ecologique)
    - reprendre couleurs de la charte graphique
    - Peut être même **Design Sonore**
    - Pas de style exclusif à suivre
    - référencement
    - Site moderne qui ne fait pas fuir direct
    - Pas de e-shop mais peut être donner la possibilité de commander via un formulaire de contact pour les EP, merchandising


5. Mettre en avant

    - Groupe **engagé** (éco conception, durabilité, écologie)
    - Rock Celtique
    - Tiennent à leurs valeurs

6. Objectif


    - Avoir un MAX de date
      - être visible
      - montrer qui est PanKart
      - style de musique
      - disponibilité   


7. Pages

    - Landing page
    - Lyrics
    - Mettre tous les liens ou on peut les retrouver (style FB IG Spotify réseaux sociaux, streaming, apps etc)
    - Page bio du Groupe
    - Page individuelle pour les chaques membres
    - page NEWS en rapport au groupe
    - Page coups de coeur et coups de gueuler (style blog ou twitter, 1 image avec texte qui l'accompagne)
    - Page photo (gallerie photo)
    - Page vidéo (gallerie vidéo)
    - Page Presse
    - Page instruments utilisés
    - Page contact
    - Page agenda de concert
    - Page booking : envoyer vers gigstarter et faire un form pour réserver
    - Page private policy (RGPD)

8. Sites inspirations (client)

    - [Foo Fighters](https://www.foofighters.com/)
    - [HolyHopCircus](https://www.holyhopcircus.be/)

***

## Plan du site 

### Élement principaux sur toutes les pages

Header :

- Logo Pankart
- Navigation vers les pages principales
- Barre de recherche (coups de gueule / coup de coeur, membres du groupe, information sur le site)

Footer :

- Lien vers des pages secondaires
- Lien vers réseaux sociaux
- Lien vers plateforme pour écouter l'EP
- Privacy Policy


### Accueil

**Objectif :**

> Présenter le groupe

> Prévoir un espace pour afficher et réserver une date ou acheter une place pour le groupe

> Voir le dernier album du groupe 

> Voir les derniers coups de gueule / coup de coeur du groupe

> Contacter le groupe pour acheter l'EP 

**Contenu :**

Section 'présentation' :

- Information sur Pankart
- Description 
- Call to action pour réserver une date

Section 'dates' :

- Titre de la section
- Dates
- Call to action vers la billeterie
- Call to action vers la page agenda

Section 'album' :

- Titre de la section
- Photo de l'album 
- Titre de l'album
- Petite description
- Call to action pour l'écouter
- Call to action vers toutes les plateformes ou l'écouter
- Call to action vers la page de contact pour acheter l'EP

Section 'Coup de gueule / Coup de coeur' :

- Titre de la section
- Titre de la news
- information de la news
- Call to action pour aimer
- Call to action pour partager
- Call to action pour commenter

### Coup de coeur / Coup de gueule

**Objectif :**

> Afficher les coups de coeur / coups de gueule

**Contenu :**

- Listes de tout les coups de coeurs / coup de gueule
    - Titre 
    - Date
    - Image s'il y en a une
    - Contenu
    - Call to action pour aimer
    - Call to action pour partager
    - Call to action pour commenter

### Page presse / News

**Objectif :**

> Partager les articles dans lesquels Pankart apparait, des sorties des albums

**Contenu :**

- Listes de tout les articles  et apparition
    - Titre
    - Date
    - Image s'il y en a une
    - Contenu
    - Call to action pour aimer
    - Call to action pour partager
    - Call to action pour commenter

### Page Biographie

**Objectif :**

> Décrire l'histoire du groupe

> Décrire l'histoire de chacun de ses membres 

> Montrer leurs albums 

**Contenu :**

Section Biographie :

- Présentation du groupe
- Photo du groupe
- Description du groupe
- Call to action vers Gigstarter
- Call to action pour écouter leurs albums

Section Biographie des membres :

- Liste des membres du groupes
    - Nom et prénom
    - Photo du membre
    - Rôle
    - Description
    
Section Album : 

- Nom de l'album
- Photo de l'album
- Date de sortie
- Description
- Call to action pour l'écouter
- Call to action vers les plateformes pour écouter l'album

#### Page d'un membre

Section présentation :

- Nom et prénom
- Rôle dans le groupe
- Photo du membre
- Call to action vers les autres membres

### Galerie

**Objectif :**

> Montrer les photos du groupe

**Contenu :**

- Listes d'images des photos et vidéos du groupe
- Call to action pour aller vers les pages 'photo' et 'vidéo'

#### Page photo

- Listes de toute les photos

#### Page vidéo

- Listes de toutes les vidéos

### Agenda

**Objectif :**

> Montrer dans l'agenda toutes les dates dans lesquels le groupe va se présenter

**Contenu :**

Section 'Agenda' :

- Agenda
    - Dates auxquels Pankart se présentera
    - Lieu
    
Section 'Date' :

- Date
- Lieu
- Description
- Call to action vers la billeterie

### Lyrics

**Objectif :**

> Montrer les lyrics des musiques

**Contenu :**

- Titre de l'album
    - Liste des musiques
    - Lyrics des musiques
    - durée de la musique

### Contact

**Objectif :**

> Contacter le groupe pour différentes actions préenregistrées

**Contenu :**

- Formulaire de contact
    - Nom et prénom
    - Adresse e-mail
    - Sujet à choix
    - Message
    
## Stratégie

### Élément principaux sur toutes les pages

**Liens :**

Tout les liens auront un effet au hover pour bien montrer que c'est un lien

**Header :**

Changement de couleurs des liens au survol, la page active sera mise en avant, dans le champs de recherches, les recherches s'afficheront au fur et à mesure que l'utilisateur écrira.

**Footer :**

Changement de couleurs des liens au survol.

### Accueil

**Section présentation :**

Une vidéo d'arrière-plan sera sur la landing page pour montrer ce que le groupe sait faire, un bouton de volume sera disponible pour agmenter et diminuer le son , un bouton mute sera aussi présent.

**Section Date :**

Les dates seront présenter différemment, en bloc et ce sera un slider pour passer de dates en dates.

**Section Coups de coeur / coup de gueule :**

Ce sera aussi un slider en montrer 2 news par 2.

### Biographie

**Section membres :**

Au survol de chaque membre, une ombre apparaitra comme pour montrer qu'elle se soulève.

### Page d'un membre

Au survol de chaque autres membres, une ombre apparaitra comme pour montrer qu'elle se soulève.

### Galerie

Un effet sera mis des le survol d'une image ou d'une vidéo

### Agenda

Lors d'un clic sur une date de l'agenda, l'évènement s'affichera avec plus d'information en dessous de l'agenda.

### Lyrics

Chaque chanson aura sa lyric, et au clic d'un titre, les lyrics s'afficheront et il y aura aussi un bouton pour jouer la musique.